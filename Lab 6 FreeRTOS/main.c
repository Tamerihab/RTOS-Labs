...
int main() {
    // Initialize hardware peripherals and FreeRTOS
	   ...
	   init();
    // Create tasks
    xTaskCreate(toggleRedLED, "RedTask", configMINIMAL_STACK_SIZE,(void *) &LED_RED, 1, NULL);
    xTaskCreate(toggleGreenLED, "GreenTask", configMINIMAL_STACK_SIZE, (void *) &LED_GREEN, 1, NULL);
...
void toggleGreenLED(void *pvParameters) {
    ...
    for (;;) {
        ...
        vTaskDelay(pdMS_TO_TICKS(2000)); // Delay 3000ms (3 seconds)
    }
}
